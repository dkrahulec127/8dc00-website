...
def linear_regression():
    ...
    #!studentstart
    Theta, e = reg.ls_solve(trainXones, trainY)
    #!studentend
    #---------------------------------------------------------------------#
...
def quadratic_regression():
    ...
    test_x = np.concatenate((testX, testX**2), axis=1)
    training_x = np.concatenate((trainX, trainX**2), axis=1)
    validation_x = np.concatenate((validationX, validationX**2), axis=1)

    testXones = util.addones(test_x)
    trainXones = util.addones(training_x)
    validationXones = util.addones(validation_x)

    Theta, e = reg.ls_solve(trainXones, trainY)

    fig1 = plt.figure(figsize=(10,10))
    ax1 = fig1.add_subplot(111)
    util.plot_regression(training_x, trainY, Theta, ax1)
    ax1.grid()
    ax1.set_xlabel('x')
    ax1.set_ylabel('y')
    ax1.legend(('Original data', 'Regression curve', 'Predicted Data', 'Error'))
    ...
    fig2 = plt.figure(figsize=(10,10))
    ax2 = fig2.add_subplot(111)
    util.plot_regression(test_x, testY, Theta, ax2)
    ax2.grid()
    ax2.set_xlabel('x')
    ax2.set_ylabel('y')
    ax2.legend(('Original data', 'Regression curve', 'Predicted Data', 'Error'))
    ...
    E_validation = np.mean((validationXones.dot(Theta) - validationY)**2)
    E_test = np.mean((testXones.dot(Theta) - testY)**2)
    #!studentend
    #---------------------------------------------------------------------#
