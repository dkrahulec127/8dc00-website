...
def nuclei_measurement():
    ...
    # training sample).
    #!studentstart
    trainXonesfull = util.addones(training_x)
    trainXones = util.addones(training_x[::4,:])
    testXones = util.addones(test_x)
    Theta, E = reg.ls_solve(trainXones, training_y[::4])
    predicted_y = testXones.dot(Theta)
    E2 = np.mean((trainXonesfull.dot(Theta) - training_y)**2)
    #!studentend
    #---------------------------------------------------------------------#
...
def nuclei_classification():
    ...
    Theta = 0.00002*np.random.rand(training_x.shape[1]+1, 1)
