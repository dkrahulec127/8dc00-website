...
def correlation(I, J):
    ...
    #!studentstart
    # Matlab: C = (u'*v) / (sqrt(u'*u)*sqrt(v'*v));
...
def mutual_information_e(p):
    ...
    #!studentstart
    H_I = -p_I.transpose().dot(np.log(p_I))
    H_J = -p_J.dot(np.log(p_J.transpose()))
    pc = p.reshape(p.shape[0]*p.shape[1], 1)
    H_IJ = -pc.transpose().dot(np.log(pc))
    MI = H_I + H_J - H_IJ
    #!studentend
    #------------------------------------------------------------------#
...
def affine_corr(I, Im, x, return_transform=True):
    ...
    #!studentstart
    # the first element is the rotation angle, the second and third are
    # the scaling parameters and the fourth and fifth are the shearing
    # parameters
    T1 = rotate(x[0])
    T2 = util.scale(x[1], x[2])
    T3 = shear(x[3], x[4])
    T = T1.dot(T2).dot(T3)

    # the remaining two elements are the translation
    Th = util.t2h(T, SCALING*x[5:])
