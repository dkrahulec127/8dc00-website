...
def combining_transforms():
    ...
    T_reflect = reg.reflect(-1, 1)

    X_t1 = T_shear.dot(T_rot).dot(X)
    X_t2 = T_reflect.dot(T_rot).dot(X)
    X_t3 = T_rot.dot(T_reflect).dot(X)
    ...
    util.plot_object(ax1, X)
    util.plot_object(ax2, X_t1)
    util.plot_object(ax3, X_t2)
    util.plot_object(ax4, X_t3)

    ax1.set_title('Original')
    ax2.set_title('Rotation + shearing')
    ax3.set_title('Rotation + reflection')
    ax4.set_title('Reflection + rotation')
...
def arbitrary_rotation():
    ...
    #!studentstart
    T_1 = util.t2h(reg.identity(), X[:,0])
    T_2 = util.t2h(reg.rotate(np.pi/4), np.zeros(2))
    T_3 = util.t2h(reg.identity(), -X[:,0])
...
def correlation_test():
    ...
    assert abs(C2 - C3) < 10e-10,"Correlation function is incorrectly implemented (swopped sign correlation test)"

    C4 = reg.correlation(I.astype(float)*2 + 10, J)
    assert abs(C2 - C4) < 10e-10,"Correlation function is incorrectly implemented (scale correlation test)"
    #!studentend
    #------------------------------------------------------------------#
...
def mutual_information_test():
    ...
    # this should be identical to the MI of the image with its "negative"
    # small difference is possible due to rounding errors
    p2 = reg.joint_histogram(I, 255-I)
    MI2 = reg.mutual_information(p2)
    assert abs(MI1-MI2) < 10e-3, "Mutual information function is incorrectly implemented (negative image test)"

    # mutual information between random noise images; it should be very close to zero
...
def ngradient_test():
    ...
    g2 = reg.ngradient(sinus, np.pi * np.ones((1,)))
    assert abs(g2 - np.cos(np.pi)) < 1e-5, "Numerical gradient is incorrectly implemented (sine test)"

    squared = lambda x: np.power(x, 2)
    g3 = reg.ngradient(squared, 2 * np.ones((1,)))
    assert abs(g3 - 4) < 1e-5, "Numerical gradient is incorrectly implemented (power test)"
    #!studentend
    #------------------------------------------------------------------#
